            ...
            adapter.setOnSelectedListener(new BaseListAdapter.OnSelectedListener<TaskRewardItem>() {
                ...
                public void onSelected(@Nullable TaskRewardItem data) {
                    if (data != null && !selected) {
                        RewardsThatWork.getInstance().setTaskId(data.getId());
                        navController.navigate(R.id.action_select);
                        selected = true;
                    }
                }
            });
            viewModel.getTaskList().observe(getViewLifecycleOwner(), new Observer<TaskRewardList>() {
                @Override
                public void onChanged(TaskRewardList taskList) {
                    adapter.submitList(taskList.getList());
                    showEmpty(taskList.isEmpty());
                    showLoading(false);
                }
            });
