...
public class TaskApiImpl implements TaskRewardApi {
    private final CollectionReference collection;

    public TaskApiImpl(@NonNull String groupId) {
        this.collection = FirebaseFirestore.getInstance().collection("groups").document(groupId).collection("tasks");
    }

    @NonNull
    @Override
    public AsyncResult<String> create(@NonNull NetworkTaskReward networkTask) {
        final AsyncResult<String> result = new AsyncResult<>();
        final DocumentReference document = collection.document();
        document.set(networkTask).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    result.setResult(document.getId());
                } else {
                    result.setError("Could not create task network data.");
                }
            }
        });
        return result;
        ...
        collection.document(id).get().continueWith(new Continuation<DocumentSnapshot, Entity<NetworkTaskReward>>() {
            ...
            public Entity<NetworkTaskReward> then(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful() && task.getResult() != null && task.getResult().exists()) {
                    NetworkTaskReward networkTask = task.getResult().toObject(NetworkTaskReward.class);
                    if (networkTask != null) {
                        return new Entity<>(task.getResult().getId(), networkTask);
                    }
                }
                return null;
            }
    ...
    public AsyncResult<List<Entity<NetworkTaskReward>>> getAll() {
        ...
        collection.orderBy("name").get().continueWith(new Continuation<QuerySnapshot, List<Entity<NetworkTaskReward>>>() {
            @Override
            public List<Entity<NetworkTaskReward>> then(@NonNull Task<QuerySnapshot> task) throws Exception {
                if (task.isSuccessful() && task.getResult() != null) {
            ...
            public void onComplete(@NonNull Task<List<Entity<NetworkTaskReward>>> task) {
                if (task.isSuccessful() && task.getResult() != null) {
                    ...
                    asyncResult.setError("Could not get list of task network data.");
                }
            }
        });
        return asyncResult;
        ...
        collection.document(id).set(networkTaskReward).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    ...
                    asyncAction.setError("Could not set task network data.");
                }
            }
        });
        return asyncAction;
